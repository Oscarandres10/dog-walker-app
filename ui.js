...
function mostrarSeccionClienteUI() {
  if (miSistema.logueado !== null) {
    ...
    while (x < miSistema.contrataciones.length && !contratado) {
      ...
      if (
        idClienteContrato === clienteId &&
        estadoContratacion !== "denegada"
      ) {
        contratado = true;
      }
      x++;
...
function darVidaBotonesTablaContratacionesPendientesUI() {
  let botonAceptar = document.querySelectorAll(
    ".botonesTablaContratacionesAceptada"
  );
  for (let unBoton of botonAceptar) {
    unBoton.addEventListener("click", clickEnAceptarUI);
  }
  let botonRechazar = document.querySelectorAll(
    ".botonesTablaContratacionesRechazada"
  );
...
function mostrarTablaContratacionesAceptadasUI() {
  ...
  document.querySelector("#mostrarTablaContratacionesAceptadas").innerHTML =
    laTabla;
}
